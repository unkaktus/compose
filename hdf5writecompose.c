...
void hdf5_write_compo_q_(int* m_nb, int*n_temp, int* o_y_q, int * n_q,double * nb, double * t, double * y_q, double * yav,double * aav, double * zav, double * nav, int * index_av,int * openfile, int* itab )
{
    hid_t       file, dataset;         /* file and dataset handles */
    hid_t       datatype, dataspace;   /* handles */
    hsize_t     dimsf[1];/* dataset dimensions */
...
/*      * Close/release resources.
    ...
    /* write points for average neutron number */
    dataspace = H5Screate_simple(RANK1, dimsfthermo, NULL);
    dataset = H5Dcreate2(file, "nav", H5T_NATIVE_DOUBLE, dataspace,
			H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    status = H5Dwrite(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, nav);
    H5Dclose(dataset);
    H5Sclose(dataspace);
