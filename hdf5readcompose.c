...
void hdf5_read_av_(double * nb, double * t, double * y_q, double * yav,double * aav, double * zav, double * nav, int * index_quantity,int * openfile)
{
    hid_t       file, dataset;         /* file and dataset handles */
    hid_t       datatype, dataspace;   /* handles */
    herr_t      status;
    ...
    /* read points for nav */
    dataset = H5Dopen2(file, "nav", H5P_DEFAULT);
    status = H5Dread(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,nav);
    H5Dclose(dataset);
